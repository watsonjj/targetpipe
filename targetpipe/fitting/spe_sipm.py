...
def pe_signal(k, x, norm, eped, eped_sigma, spe, spe_sigma, lambda_, opct, pap, dap):
    ...
    ap_sigma = np.sqrt(k * s1ap ** 2 + eped_sigma ** 2)
    signal = norm * pap_noap[k] * gaussian.pdf(x, pe, pe_sigma)
    signal += norm * pap_ap1[k] * gaussian.pdf(x, eped+n[1:]*spe*(1.0-d1ap), pe_sigma)
    signal += norm * pap_ap2[k] * gaussian.pdf(x, eped+n[1:]*spe*(1.0-d2ap), ap_sigma)
